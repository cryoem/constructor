...
$ export PATH=$PREFIX/bin:\$PATH; export PYTHONPATH=$PREFIX/lib:\$PYTHONPATH; export EMAN2DIR=$PREFIX
"
    else
        if [ -f $BASH_RC ]; then
            echo "
...
export PATH=\"$PREFIX/bin:\$PATH\"; export PYTHONPATH=\"$PREFIX/lib:\$PYTHONPATH\"; export EMAN2DIR=\"$PREFIX\"" >>$BASH_RC
    fi
